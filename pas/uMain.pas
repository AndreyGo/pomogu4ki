  ...
  if MessageBox(Application.MainForm.Handle,'You realy want to delete library ?','Delete library!',MB_YESNO + MB_ICONQUESTION) = ID_YES then
     Begin
       ...
       if DeleteFile(PLibrary(lvDllList.Selected.Data).DllPath) then
          ...
          Begin
            LAST_ERROR:= ERROR_MAIN_CANT_DELETE;
            ShowErrorMessage;
          End;
     End;
end;
...
begin
  ...
  if lvDllList.Selected = nil then exit;
  PLibrary(lvDllList.Selected.Data).Settings;
end;
...
begin
  ...
  if lvDllList.Selected = nil then exit;
  sInfo:= '/************************** Detail info about "' +  PLibrary(lvDllList.Selected.Data).DllInfo.DllName + '" **************************\'#13#13;
  sInfo:= sInfo + 'Author: ' + PLibrary(lvDllList.Selected.Data).DllInfo.DllExtension.Author + #13#13;
  sInfo:= sInfo + 'E-Mail: ' + PLibrary(lvDllList.Selected.Data).DllInfo.DllExtension.Email + #13#13;
  sInfo:= sInfo + 'Blog: ' + PLibrary(lvDllList.Selected.Data).DllInfo.DllExtension.Blog + #13#13;
  sInfo:= sInfo + 'Description: ' + PLibrary(lvDllList.Selected.Data).DllInfo.DllDescription + #13#13#13;
  sInfo:= sInfo + '--------------------------------------'#13;
  sInfo:= sInfo + 'Control Panel created by Andrey_Go (c)';
...
begin
  ...
  Lib.Initialize;
  ICO:= TIcon.Create;
  ICO.Handle:= LoadIconW(Lib.DllHandle,'DllIco');
  New(pData);
  with lvDllList.Items.Add do
   Begin
      ...
      pData^ := lib;
      if ICO.Handle > 0 then ImageIndex:= ImageList.AddIcon(ICO);
      Data:= pData;
   End;
   ICO.Free;
end;
...
begin
  ...
  if not DirectoryExists(ROOT_DIR  + LIBRARIES_DIR) then
     Begin
       LAST_ERROR:= ERROR_MAIN_LIBDIR_NOTFOUND;
       exit;
     End;
...
begin
  ...
  case LAST_ERROR of
    0: sMessage:= 'Libraries directory not found';
    ERROR_MAIN_LIBDIR_NOTFOUND:sMessage:= 'Directory with libraries not found.';
    ERROR_MAIN_CANT_DELETE: sMessage:= 'Can''t delete a file.';
  end;
